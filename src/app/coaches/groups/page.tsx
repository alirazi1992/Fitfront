...
const GROUP_SERIES = [
  {
    title: "Hybrid strength cohorts",
    description: "Blend in-person lifts with remote velocity tracking and recovery labs.",
  },
  {
    title: "Conditioning pods",
    description: "Interval-based squads powered by live heart-rate tiles and auto-scaled targets.",
  },
  {
    title: "Corporate resilience",
    description: "Eight-week durability sprints for distributed teams with shared leaderboards.",
  },
];
...
const WORKFLOWS = [
  {
    phase: "Assemble",
    detail: "Select templates, assign coaches, and lock capacity. The system balances experience levels instantly.",
  },
  {
    phase: "Launch",
    detail: "Push schedules, content drops, and equipment lists in a single broadcast feed.",
  },
  {
    phase: "Iterate",
    detail: "Live analytics highlight adherence so you can split cohorts or deploy support coaches in seconds.",
  },
];
...
const METRICS = [
  {
    ...
    value: "42",
    helper: "Running this quarter",
  },
  {
    label: "Avg fill rate",
    value: "92%",
    helper: "Across all group types",
  },
  {
    label: "Coach coverage",
    value: "3.1",
    helper: "Coaches per cohort",
  },
];
    ...
    <div className="flex min-h-full flex-col gap-12 bg-[var(--color-surface)] px-6 py-10 text-sm text-[color:var(--color-text-secondary)] lg:px-12 lg:py-16">
      <div className="flex flex-col gap-6 lg:flex-row">
        ...
        <section className="relative overflow-hidden rounded-[3rem] border border-white/10 bg-[var(--color-shell)]/80 px-8 py-12 shadow-[0_40px_120px_-45px_rgba(12,15,24,0.8)] backdrop-blur-2xl lg:px-16 lg:py-16">
          ...
          <div className="absolute -top-32 left-1/3 h-72 w-72 rounded-full bg-[var(--color-accent-blue)]/40 blur-[140px]" />
          <div className="absolute -bottom-32 right-[18%] h-80 w-80 rounded-full bg-[var(--color-accent-yellow)]/35 blur-[150px]" />
        ...
        <div className="grid gap-12 lg:grid-cols-[1.1fr_0.9fr]">
          <div className="space-y-8">
            <div className="flex flex-wrap items-center gap-3 text-xs uppercase tracking-[0.55em] text-[var(--color-accent-blue)]">
              ...
              <span className="inline-flex items-center gap-2 rounded-full border border-white/15 bg-white/5 px-4 py-2 text-white/70">
                6,120 athlete slots this month
              </span>
            </div>
            <div className="space-y-6">
              <h1 className="text-4xl font-semibold tracking-tight text-[var(--color-text-primary)] sm:text-5xl lg:text-6xl">
                Launch cohorts that feel bespoke, even at scale
              </h1>
              <p className="max-w-2xl text-base">
                Orchestrate hybrid classes, remote squads, and executive programs with a command center built for velocity.
                Every update broadcasts instantly across chat, calendar, and content.
              </p>
            </div>
            ...
            <div className="grid gap-4 sm:grid-cols-3">
              {METRICS.map((metric) => (
                ...
                >
                  <span className="text-[11px] uppercase tracking-[0.5em] text-white/60">{metric.label}</span>
          ...
          <aside className="relative flex h-full flex-col gap-6 overflow-hidden rounded-[2.5rem] border border-white/10 bg-[var(--color-panel)]/40 p-8 text-xs text-[color:var(--color-text-secondary)] shadow-[0_30px_80px_-40px_rgba(12,15,24,0.9)]">
            ...
            <div className="flex items-center justify-between text-sm text-[var(--color-text-primary)]">
              <p className="font-semibold uppercase tracking-[0.45em]">Series spotlight</p>
              <span className="rounded-full border border-white/10 bg-white/10 px-3 py-1 text-[11px] text-white/70">Live</span>
            </div>
            <div className="space-y-4 text-sm">
              {GROUP_SERIES.map((series) => (
                <article
                  key={series.title}
                  className="group relative overflow-hidden rounded-[1.8rem] border border-white/10 bg-white/5 px-5 py-5"
                >
                  <h3 className="text-base font-semibold text-white">{series.title}</h3>
                  <p className="mt-2 text-xs text-white/60">{series.description}</p>
      ...
      <Panel className="relative overflow-hidden border-white/10 bg-[var(--color-shell)]/70 px-8 py-10 shadow-[0_30px_90px_-50px_rgba(12,15,24,0.9)] lg:px-12">
        ...
        <div className="grid gap-6 lg:grid-cols-3">
          {WORKFLOWS.map((workflow) => (
            ...
            >
              <span className="text-xs uppercase tracking-[0.45em] text-[var(--color-accent-blue)]">{workflow.phase}</span>
              <p className="mt-3 text-base font-semibold text-white">{workflow.phase}</p>
              <p className="mt-2 text-sm text-white/60">{workflow.detail}</p>
            </div>
          ))}
        </div>
      </Panel>
