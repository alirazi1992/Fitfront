...
const THREADS = [
  {
    name: "Team Atlas",
    status: "New feedback",
    detail: "Athletes shared video form checks for tomorrow's session.",
  },
  {
    name: "Recovery Lab",
    status: "Automation",
    detail: "Breathwork protocol dispatched post heavy pulls.",
  },
  {
    name: "Corporate squad",
    status: "Live",
    detail: "Midday mobility stream is running with 54 attendees.",
  },
];

const QUICK_REPLIES = [
  "Share updated warm-up",
  "Send check-in survey",
  "Schedule video recap",
    ...
    <div className="flex min-h-full flex-col gap-12 bg-[var(--color-surface)] px-6 py-10 text-sm text-[color:var(--color-text-secondary)] lg:px-12 lg:py-16">
        ...
        <div className="grid gap-12 lg:grid-cols-[1.1fr_0.9fr]">
          <div className="space-y-8">
            <div className="flex flex-wrap items-center gap-3 text-xs uppercase tracking-[0.55em] text-[var(--color-accent-blue)]">
              ...
              <span className="inline-flex items-center gap-2 rounded-full border border-white/15 bg-white/5 px-4 py-2 text-white/70">
                Response time 6m avg
              </span>
            </div>
            <div className="space-y-6">
              <h1 className="text-4xl font-semibold tracking-tight text-[var(--color-text-primary)] sm:text-5xl lg:text-6xl">
                Keep conversations, content, and automations in one stream
              </h1>
              <p className="max-w-2xl text-base">
                Coaches, clients, and groups collaborate with threads tied directly to sessions and templates. Automations can
                chime in, but humans stay in control.
              </p>
            </div>
          ...
          <aside className="relative flex h-full flex-col gap-6 overflow-hidden rounded-[2.5rem] border border-white/10 bg-[var(--color-panel)]/40 p-8 text-xs text-[color:var(--color-text-secondary)] shadow-[0_30px_80px_-40px_rgba(12,15,24,0.9)]">
            ...
            <div className="flex items-center justify-between text-sm text-[var(--color-text-primary)]">
              <p className="font-semibold uppercase tracking-[0.45em]">Active threads</p>
              <span className="rounded-full border border-white/10 bg-white/10 px-3 py-1 text-[11px] text-white/70">Streaming</span>
            </div>
              ...
              {THREADS.map((thread) => (
                ...
                >
                  <div className="flex items-center justify-between">
                    ...
                    <span className="text-[11px] text-white/60">{thread.status}</span>
                  </div>
                  <p className="mt-2 text-xs text-white/60">{thread.detail}</p>
            ...
            <div className="space-y-3 rounded-[1.8rem] border border-white/10 bg-white/5 px-6 py-5 text-sm text-white/70">
              <p className="text-xs uppercase tracking-[0.45em] text-[var(--color-accent-blue)]">Quick replies</p>
              <div className="flex flex-wrap gap-2">
                {QUICK_REPLIES.map((reply) => (
                  <button
                    key={reply}
                    type="button"
                    className="rounded-full border border-white/15 bg-white/5 px-4 py-2 text-xs font-semibold text-white/70 transition hover:border-[var(--color-accent-blue)] hover:text-white"
                  >
                    {reply}
                  </button>
                ))}
      ...
      <Panel className="relative overflow-hidden border-white/10 bg-[var(--color-shell)]/70 px-8 py-10 shadow-[0_30px_90px_-50px_rgba(12,15,24,0.9)] lg:px-12">
        ...
        <div className="grid gap-6 lg:grid-cols-3">
          <div className="rounded-[1.9rem] border border-white/10 bg-white/5 px-6 py-6 text-sm text-white/70">
            <span className="text-xs uppercase tracking-[0.45em] text-[var(--color-accent-blue)]">Voice rooms</span>
            <p className="mt-3 text-sm text-white/60">Spin up drop-in coaching sessions or Q&A lounges with whiteboard mode.</p>
          </div>
          <div className="rounded-[1.9rem] border border-white/10 bg-white/5 px-6 py-6 text-sm text-white/70">
            <span className="text-xs uppercase tracking-[0.45em] text-[var(--color-accent-blue)]">Media sync</span>
            <p className="mt-3 text-sm text-white/60">Send videos and documents from the template library without leaving chat.</p>
          </div>
          <div className="rounded-[1.9rem] border border-white/10 bg-white/5 px-6 py-6 text-sm text-white/70">
            <span className="text-xs uppercase tracking-[0.45em] text-[var(--color-accent-blue)]">Automation guardrails</span>
            <p className="mt-3 text-sm text-white/60">Approve or edit AI-suggested replies before they reach clients.</p>
          </div>
        </div>
      </Panel>
    </div>
